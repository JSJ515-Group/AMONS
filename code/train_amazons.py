...
class BPR(nn.Module):
    ...
    def forward(self, n_fold, num_negs):
        ...
        for step, batch_edges in enumerate(train_edges_data_loader):
            mf_losses = compute_info_bpr_loss(gcn_users_embedding, gcn_items_embedding, batch_edges, combined_user_item_dict, num_negs,
                                              reduction="none")
            l2_loss = compute_l2_loss([gcn_users_embedding, gcn_items_embedding])
